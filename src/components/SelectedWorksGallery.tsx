...
export default function SelectedWorksGallery({
  ...
  return (
    <>
      ...
      {/* Sezione CUMULI */}
      {cumuliImages.length > 0 && (
        <section className="mb-24">
          <h2 className="mb-12 text-4xl font-normal uppercase tracking-wide text-neutral-900">
            Cumuli
          </h2>
          <ul className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {cumuliImages.map((src) => (
              <li key={src} className="overflow-hidden rounded-lg">
                <button
                  type="button"
                  onClick={() => setFullImage(src)}
      ...
      {open && (
        ...
        >
          ...
          <button
            ...
            className="absolute top-4 right-4 z-20 flex h-8 w-8 items-center justify-center text-white/90 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
            aria-label="Chiudi"
          >
